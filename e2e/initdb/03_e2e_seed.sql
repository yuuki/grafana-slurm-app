...
INSERT INTO `gpu_cluster_job_table`
  (`id_job`, `id_assoc`, `id_user`, `id_group`, `job_name`, `partition`,
   ...
   `exit_code`, `priority`, `work_dir`, `tres_alloc`, `tres_req`, `failed_node`)
VALUES
-- Timeout job
...
SELECT
  ...
  2,
  64,
  UNIX_TIMESTAMP() - 60000 - (seq.n * 60),
  UNIX_TIMESTAMP() - 59950 - (seq.n * 60),
  UNIX_TIMESTAMP() - 59900 - (seq.n * 60),
  IF(MOD(seq.n, 4) = 0, UNIX_TIMESTAMP() - 59700 - (seq.n * 60), 0),
  0,
  4294000000,
